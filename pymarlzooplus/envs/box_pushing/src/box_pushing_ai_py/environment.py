...
class BoxPushing(gym.Env):
    ...
    def reset(self, seed=None, options=None, debug=False):
        ...
        # Dictionary returned for compatibility with gymnasium
        return self._getobs(), {}
    ...
    def step(self, actions, debug=False):
        ...
        if (
                (actions[0] == 0) and (actions[1] == 0) and self.agents[0].ori == 0 and self.agents[1].ori == 0 and
                (
                        (
                                ...
                                self.agents[1].ycoord == self.boxes[2].ycoord - 1.0
                        ) or
                        (
                                self.agents[1].xcoord == self.boxes[2].xcoord - 0.5 and
                                self.agents[0].xcoord == self.boxes[2].xcoord + 0.5 and
        ...
        return (
            observations,
            [rewards] * self.n_agent,
            int(terminate or self.count_step == self.terminate_step),
            [False] * self.n_agent,  # Just for compatibility with gymnasium
            {}
        )
    ...
    def _getobs(self, debug=False):
        ...
        for idx, agent in enumerate(self.agents):
            ...
            # observe large box
            if (
                    self.boxes[2].xcoord + 0.5 == agent.xcoord + DIRECTION[agent.ori][0] or
            ...
            # observe wall
            if (
                    agent.xcoord + DIRECTION[agent.ori][0] > self.xlen or
            ...
            if idx == 0:
                ...
                teamate_idx = 0
            if (
                    (agent.xcoord + DIRECTION[agent.ori][0] == self.agents[teamate_idx].xcoord) and
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            ...
            self.small_box_0_trans = rendering.Transform(translation=(
                self.boxes[0].xcoord * 100 * scale, self.boxes[0].ycoord * 100 * scale
            ))
            ...
            self.small_box_0_in_trans = rendering.Transform(translation=(
                self.boxes[0].xcoord * 100 * scale, self.boxes[0].ycoord * 100 * scale
            ))
            ...
            self.small_box_1_trans = rendering.Transform(translation=(
                self.boxes[1].xcoord * 100 * scale, self.boxes[1].ycoord * 100 * scale
            ))
            ...
            self.big_box_2_trans = rendering.Transform(translation=(
                self.boxes[2].xcoord * 100 * scale, self.boxes[2].ycoord * 100 * scale
            ))
            big_box_2.add_attr(self.big_box_2_trans)
            self.viewer.add_geom(big_box_2)

            big_box_2_in = rendering.FilledPolygon([
            ...
            self.big_box_2_in_trans = rendering.Transform(translation=(
                self.boxes[2].xcoord * 100 * scale, self.boxes[2].ycoord * 100 * scale
            ))
            ...
            self.agent_0_trans = rendering.Transform(translation=(
                self.agents[0].xcoord * 100 * scale, self.agents[0].ycoord * 100 * scale
            ))
            ...
            self.agent_0_in_trans = rendering.Transform(translation=(
                self.agents[0].xcoord * 100 * scale, self.agents[0].ycoord * 100 * scale
            ))
            ...
            self.agent_1_trans = rendering.Transform(translation=(
                self.agents[1].xcoord * 100 * scale, self.agents[1].ycoord * 100 * scale
            ))
            ...
            self.agent_1_in_trans = rendering.Transform(translation=(
                self.agents[1].xcoord * 100 * scale, self.agents[1].ycoord * 100 * scale
            ))
            ...
            sensor_0_in = rendering.FilledPolygon([
            ...
            sensor_1_in = rendering.FilledPolygon([
                (-sensor_in_size / 2.0 * scale, -sensor_in_size / 2.0 * scale),
                (-sensor_in_size / 2.0 * scale, sensor_in_size / 2.0 * scale),
                (sensor_in_size / 2.0 * scale, sensor_in_size / 2.0 * scale),
                (sensor_in_size / 2.0 * scale, -sensor_in_size / 2.0 * scale)
