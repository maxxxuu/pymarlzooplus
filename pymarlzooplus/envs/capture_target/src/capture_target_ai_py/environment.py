...
class CaptureTarget(gym.Env):
    ...
    def __init__(
        ...
        for i in range(self.n_agent):
            assert self.n_action[i] == self.n_action[0]
        self.action_space = spaces.Discrete(self.n_action[0])
        ...
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(self.obs_size[0],), dtype=np.float32)
    ...
    def action_space_sample(self, idx):
        return self.np_random.integers(self.n_action[idx])

    def action_space_batch_sample(self, idx):
        return self.np_random.integers(self.n_action[idx], size=self.n_agent)
    ...
    def step(self, actions, debug=False):
        ...
        if not self.target_captured():
            if not self.tgt_avoid_agent:
                target_directions = self.np_random.integers(len(TRANSLATION_TABLE), size=self.n_target)
            else:
        ...
        return (
            self.get_obs(debug),
            [r] * self.n_agent,
            int(won or self.step_n >= self.terminate_step),
            [False] * self.n_agent,  # Truncated just for compatibility with gymnasium
            {}
        )
    ...
    def rand_position(self):
        return np.array([self.np_random.integers(self.x_len), self.np_random.integers(self.y_len)])
