...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(
        ...
        # configure spaces
        self.action_space = [] # type: ignore[override]
        self.observation_space = [] # type: ignore[override]
        ...
        if self.shared_viewer:
            self.viewers: Optional[List[Viewer], List[None]] = [None]
        else:
            self.viewers: Optional[List[Viewer], List[None]] = [None] * self.n
        self._reset_render()
    ...
    def step(
        ...
        if hasattr(self.action_space, 'spaces'):
            action_space = self.action_space.spaces
        else:
            raise AttributeError("The observation space must be a Tuple of spaces")

        one_hot_actions = []
        for act, acsp in zip(action_n, action_space):
        ...
        truncated = [False] * len(done_n)

        return obs_n, reward_n, done_n, truncated, info_n
