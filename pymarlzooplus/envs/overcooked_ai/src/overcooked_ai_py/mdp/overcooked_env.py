...
class OvercookedEnv(object):
    ...
    def from_mdp(
        ...
        """
        Create an OvercookedEnv directly from an OvercookedGridworld mdp
    ...
    def step(
        ...
        truncated = [False, False]  # Always 2 agents

        # Sum rewards together
        timestep_sparse_reward = sum(mdp_infos["sparse_reward_by_agent"])

        return next_state, timestep_sparse_reward, done, truncated, env_info
...
class OvercookedEnvPettingZoo(ParallelEnv):
    ...
    def step(self, joint_action):
        ...
        def create_obs_dict(_obs):
            """
            Observation is potentially different for each agent
            """
            return {
                agent: self.agent_map[agent].featurize(_obs)
                for agent in self.agents
            }
    ...
    def reset(self, seed=None, options=None):
        """
        Reset the embedded OvercookedEnv environment to the starting state
        """
        ...
        # return the observations as dict
        obs_dict = {
...
class Overcooked(gym.Env):
    """
    Wrapper for the Env class above that is SOMEWHAT compatible with the standard gymnasium API.
    Why only somewhat? Because we need to flatten a multi-agent env to be a single-agent env (as gymnasium requires).
    ...
    def step(self, action):
        ...
        next_state, reward, done, truncated, env_info = self.base_env.step(joint_action)
