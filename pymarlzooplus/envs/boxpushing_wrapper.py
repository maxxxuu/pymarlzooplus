...
class _BoxPushingWrapper(MultiAgentEnv):
    ...
    def __init__(self, key, seed=1, **kwargs):
        ...
        if hasattr(self.original_env.unwrapped, 'seed'):
            ...
            raise AttributeError(f"'seed' attribute not found in the Box Pushing environment with key: {key}")

        # Get the timelimit
        if hasattr(self.original_env.unwrapped, 'terminate_step'):
            self.episode_limit = self.original_env.unwrapped.terminate_step
        else:
            raise AttributeError(f"'terminate_step' attribute not found in the Box Pushing environment with key: {key}")

        # TimeLimit wrapper
        self._env = TimeLimitBP(self.original_env, max_episode_steps=self.episode_limit)
        ...
        if hasattr(self._env.observation_space, 'n'):
            self._obs_size = self._env.observation_space.n
        else:
            raise AttributeError(
                f"'n' attribute not found in the observation space of the Box Pushing environment with key: {key}"
            )
        ...
        if hasattr(self._env.action_space, 'n'):
            ...
            raise AttributeError(
                f"'n' attribute not found in the action space of the Box Pushing environment with key: {key}"
            )
        ...
        if hasattr(self.original_env.unwrapped, 'obs_size'):
            ...
            raise AttributeError(f"'obs_size' attribute not found in the Box Pushing environment with key: {key}")
        if hasattr(self.original_env.unwrapped, 'n_action'):
            ...
            raise AttributeError(f"'n_action' attribute not found in the Box Pushing environment with key: {key}")
