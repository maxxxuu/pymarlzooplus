...
class ObservationPZ(ObservationWrapper):
    ...
    def observation(self, observations):
        ...
        if isinstance(observations, tuple): # It has been called from gym reset() and it carries pettingzoo observations and info
        ...
        if self.is_image and self.trainable_cnn is False:
            ...
            observations_ = [self.transform(image=observation_)["image"].detach().cpu().numpy() for observation_ in observations.values()]
            # observations_ = self.torch.concatenate(observations_, dim=0).detach().cpu().numpy()
        else:
            raise NotImplementedError("Only images are supported!")
...
class _PettingZooWrapper(MultiAgentEnv):
    ...
    def set_environment(self,
        ...
        ## Define the longest action space
        ...
        if env_name == "pistonball_v6":
            ...
            if 'n_pistons' in kwargs.keys():
                assert kwargs['n_pistons'] >= 4, \
                    "'n_pistons' argument must be greater than or equal to 4!" # Otherwise, the game stops almost immediately.
