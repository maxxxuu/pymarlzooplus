...
class CaptureTarget(gym.Env):
    ...
    def step(self, actions, debug=False):
        ...
        return self.get_obs(debug), [r] * self.n_agent, int(won or self.step_n >= self.terminate_step), {}
    ...
    def get_obs(self, debug=False):
        ...
        if debug:
            ...
            for i in range(self.n_agent):
                ...
                print("          " + " \t tgt_loc  \t\t{}".format(
                    self.target_positions[0] if not all(tgt_pos_obs[i] == -1.0) else np.array([-1, -1])))
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            ...
            for i in range(self.n_agent):
                ...
                agent_trans = rendering.Transform(translation=(
                (self.agent_positions[i][0] + 0.5) * 100 * scale, (self.agent_positions[i][1] + 0.5) * 100 * scale))
            ...
            tgt_l = rendering.FilledPolygon(
                [(-target_w / 2.0 * scale, -target_l / 2.0 * scale), (-target_w / 2.0 * scale, target_l / 2.0 * scale),
                 (target_w / 2.0 * scale, target_l / 2.0 * scale), (target_w / 2.0 * scale, -target_l / 2.0 * scale)])
            tgt_l.set_color(*target_clrs[0])
            self.tgt_l_trans = rendering.Transform(translation=tuple((self.target_positions[0] + 0.5) * 100 * scale),
                                                   rotation=np.pi / 4)
            ...
            self.tgt_r_trans = rendering.Transform(translation=tuple((self.target_positions[0] + 0.5) * 100 * scale),
                                                   rotation=-np.pi / 4)
            tgt_r.add_attr(self.tgt_r_trans)
            self.viewer.add_geom(tgt_r)

            # -------------------draw target----contours
            tgt_l = rendering.FilledPolygon([(-target_in_w / 2.0 * scale, -target_in_l / 2.0 * scale),
                                             (-target_in_w / 2.0 * scale, target_in_l / 2.0 * scale),
                                             (target_in_w / 2.0 * scale, target_in_l / 2.0 * scale),
                                             (target_in_w / 2.0 * scale, -target_in_l / 2.0 * scale)])
            tgt_l.set_color(*target_clrs[1])
            self.tgt_lc_trans = rendering.Transform(translation=tuple((self.target_positions[0] + 0.5) * 100 * scale),
                                                    rotation=np.pi / 4)
            tgt_l.add_attr(self.tgt_lc_trans)
            self.viewer.add_geom(tgt_l)

            tgt_r = rendering.FilledPolygon([(-target_in_w / 2.0 * scale, -target_in_l / 2.0 * scale),
                                             (-target_in_w / 2.0 * scale, target_in_l / 2.0 * scale),
                                             (target_in_w / 2.0 * scale, target_in_l / 2.0 * scale),
                                             (target_in_w / 2.0 * scale, -target_in_l / 2.0 * scale)])
            tgt_r.set_color(*target_clrs[1])
            self.tgt_rc_trans = rendering.Transform(translation=tuple((self.target_positions[0] + 0.5) * 100 * scale),
                                                    rotation=-np.pi / 4)
        ...
        self.tgt_l_trans.set_translation((self.target_positions[0][0] + 0.5) * 100 * scale,
                                         (self.target_positions[0][1] + 0.5) * 100 * scale)
        self.tgt_r_trans.set_translation((self.target_positions[0][0] + 0.5) * 100 * scale,
                                         (self.target_positions[0][1] + 0.5) * 100 * scale)
        self.tgt_lc_trans.set_translation((self.target_positions[0][0] + 0.5) * 100 * scale,
                                          (self.target_positions[0][1] + 0.5) * 100 * scale)
        self.tgt_rc_trans.set_translation((self.target_positions[0][0] + 0.5) * 100 * scale,
                                          (self.target_positions[0][1] + 0.5) * 100 * scale)

        for i in range(self.n_agent):
            self.render_agents[i].set_translation((self.agent_positions[i][0] + 0.5) * 100 * scale,
                                                  (self.agent_positions[i][1] + 0.5) * 100 * scale)
        for i in range(self.n_agent):
            self.render_agents[i + 2].set_translation((self.agent_positions[i][0] + 0.5) * 100 * scale,
                                                      (self.agent_positions[i][1] + 0.5) * 100 * scale)
